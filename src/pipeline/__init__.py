from src.pipeline.a_compute_xy import compute_xy_center, compute_xy_velocity, compute_xy_acceleration
from src.pipeline.b_compute_polar import compute_polar_velocity, compute_polar_acceleration
from src.pipeline.c_compute_distance_factor import compute_distance_factor, compute_non_lineal_distance_factor
from src.pipeline.d_compute_bounces import compute_bounces, compute_bounces_xy, compute_bounces_ang
from src.pipeline.e_shift_bounces import shift_detected_bounces
from src.pipeline.f_merge_bounces import merge_detected_bounces
from src.pipeline.g_post_process import post_process
...
def run_pipeline_full(df, xw=1, yw=1, xvw=1, yvw=1, xaw=1, yaw=1, avw=1, mvw=1, aaw=1, maw=1, mf=0.5, dfx=1, dfy=1, dfa=1, dfm=1, xat=30, yat=30, aat=30, mat=30, sha=0, mew=3, sb=0.5):
    """ Run the full data processing pipeline on the input dataframe.
    Parameters:
        df (pd.DataFrame): Input DataFrame containing ball position data.
        xvw (int): Window size for smoothing x velocity.
